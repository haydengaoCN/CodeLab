//describe:for the time of touching-fish : level triger v.s. edge triger
//date:20210205
//author:haydengao
//ref: https://juejin.cn/post/6844903878685622285
...
int events_handle_level(int epfd, struct epoll_event ev) {
    printf("events_handle_level, ev.events = %d\n", ev.events);
    int fd = ev.data.fd;
...
int events_handle_edge(int epfd, struct epoll_event ev) {
    printf("events_handle_edge, ev.events = %d\n", ev.events);
    int fd = ev.data.fd;
    if (ev.events == EPOLLIN) {
